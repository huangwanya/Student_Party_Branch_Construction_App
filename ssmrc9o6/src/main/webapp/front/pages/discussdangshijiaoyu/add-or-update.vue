	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussdangshijiaoyu`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussdangshijiaoyu`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussdangshijiaoyu`, this.ruleForm);
						}else{
							await this.$api.add(`discussdangshijiaoyu`, this.ruleForm);
						}
						uni.setStorageSync('discussdangshijiaoyuCleanType',true);
