<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="党费金额" prop="dangfeijine">
          <el-input v-model="ruleForm.dangfeijine" 
              placeholder="党费金额" clearable  :readonly="ro.dangfeijine"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="党费金额" prop="dangfeijine">
              <el-input v-model="ruleForm.dangfeijine" 
                placeholder="党费金额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="党员账号" prop="dangyuanzhanghao">
          <el-select :disabled="ro.dangyuanzhanghao" @change="dangyuanzhanghaoChange" v-model="ruleForm.dangyuanzhanghao" placeholder="请选择党员账号">
            <el-option
                v-for="(item,index) in dangyuanzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.dangyuanzhanghao" label="党员账号" prop="dangyuanzhanghao">
              <el-input v-model="ruleForm.dangyuanzhanghao" 
                placeholder="党员账号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="党员姓名" prop="dangyuanxingming">
          <el-input v-model="ruleForm.dangyuanxingming" 
              placeholder="党员姓名" clearable  :readonly="ro.dangyuanxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="党员姓名" prop="dangyuanxingming">
              <el-input v-model="ruleForm.dangyuanxingming" 
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/dangyuan/dangyuanzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.dangyuanzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    dangyuanzhanghaoChange () {
      this.$http({
        url: `follow/dangyuan/dangyuanzhanghao?columnValue=`+ this.ruleForm.dangyuanzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.dangyuanxingming){
                this.ruleForm.dangyuanxingming = data.data.dangyuanxingming
              }
        } else {
          this.$message.error(data.msg);
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangfeijiaojiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangfeijiaojiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
