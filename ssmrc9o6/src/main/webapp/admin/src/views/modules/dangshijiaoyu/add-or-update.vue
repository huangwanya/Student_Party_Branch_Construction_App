    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="党史内容" prop="dangshineirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.dangshineirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.dangshineirong" label="党史内容" prop="dangshineirong">
                    <span v-html="ruleForm.dangshineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	clicknum : false,
      ...
      rules: {
          dangshibiaoti: [
                { required: true, message: '党史标题不能为空', trigger: 'blur' },
          ],
          fengmian: [
          ],
          jianjie: [
  ...
  methods: {
	...
	this.ruleForm.dangshineirong = this.ruleForm.dangshineirong.replace(reg,'../../../ssmrc9o6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangshijiaoyu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangshijiaoyuCrossAddOrUpdateFlag = false;
						 this.parent.search();
