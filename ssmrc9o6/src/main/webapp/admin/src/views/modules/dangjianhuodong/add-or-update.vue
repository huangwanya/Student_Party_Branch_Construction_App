...
export default {
  ...
  methods: {
	...
	this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../ssmrc9o6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dangjianhuodong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dangjianhuodongCrossAddOrUpdateFlag = false;
						 this.parent.search();
